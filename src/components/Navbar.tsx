...
export default function Navbar() {
  ...
  return (
    ...
    >
      ...
      >
        <div className="bg-[#0a0a0a] border-t border-white/[0.06] px-6 py-5 space-y-1">
          <Link
            href="/"
            onClick={() => setIsOpen(false)}
            className="flex items-center gap-3 py-2.5 text-white/70 font-medium text-lg hover:text-white transition-colors"
          >
            {/* eslint-disable-next-line @next/next/no-img-element */}
          ...
          <a
            ...
            rel="noopener"
            onClick={() => setIsOpen(false)}
            className="flex items-center gap-3 py-3 my-1 px-3.5 rounded-xl border border-[#d2613a]/30 bg-[#d2613a]/[0.07] hover:bg-[#d2613a]/[0.14] transition-colors"
          >
            {/* eslint-disable-next-line @next/next/no-img-element */}
          ...
          {navLinks.map((link) => (
            <Link
              ...
              href={link.href}
              onClick={() => setIsOpen(false)}
              className="block py-2.5 text-white/70 font-medium text-lg hover:text-white transition-colors"
            >
              {link.label}
            </Link>
          ))}
